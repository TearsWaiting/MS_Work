...
public class TextOnlineDemo extends JFrame implements ActionListener, DocumentListener {
	// 菜单
			...
			if (isTextChange) {
				...
				if (saveChoose == JOptionPane.YES_OPTION) {
					...
					if (saveFileName == null || saveFileName.getName().equals("")) {
						...
						try {
							...
							oldValue = editArea.getText();
							this.setTitle(saveFileName.getName() + " - 记事本");
							statusLabel.setText("1当前打开文件：" + saveFileName.getAbsoluteFile());
						} catch (IOException ioException) {
						}
					}
				} else if (saveChoose == JOptionPane.NO_OPTION) {
					...
					if (fileName == null || fileName.getName().equals("")) {
						...
						try {
							...
							statusLabel.setText("2当前打开文件：" + fileName.getAbsoluteFile());
							fr.close();
							isNewFile = false;
				...
				if (fileName == null || fileName.getName().equals("")) {
					...
					try {
						...
						this.setTitle(fileName.getName() + ".txt - 记事本");
						statusLabel.setText("3当前打开文件：" + fileName.getAbsoluteFile());
						fr.close();
						isNewFile = false;
						currentFile = fileName;
						oldValue = editArea.getText();
	...
	public static void main(String args[]) {
		TextOnlineDemo textOnlineDemo = new TextOnlineDemo();
		textOnlineDemo.setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE);// 使用 System exit 方法退出应用程序
	}// main函数结束
}
